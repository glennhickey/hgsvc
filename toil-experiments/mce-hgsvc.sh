...
usage() {
    ...
    printf "Usage: $0 [OPTIONS] <JOBSTORE-NAME> <OUTSTORE-NAME> <INDEX-BASE> <SAMPLE> <NAME> <TRUE-VCF> <REGIONS> <FASTQ1> [FASTQ2]\n"
	 ...
	 printf "   -M      options for map-hgsvc.sh in quotes (SKIP to skip)\n"
	 printf "   -C      options for call-hgsvc.sh in quotes (SKIP to skip)\n"
	 printf "   -E      options for eval-hgsvc.sh in quotes (SKIP to skip)\n"
...
if [ "${MAP_OPTS}" != "SKIP" ]
then
	 ./map-hgsvc.sh $HEAD_NODE_OPTS $MAP_OPTS $JOBSTORE_NAME "${OUTSTORE_NAME}/map-${NAME}" $INDEX_BASE $SAMPLE $READS1 $READS2
fi

if [ "${CALL_OPTS}" != "SKIP" ]
then
	 ./call-hgsvc.sh $HEAD_NODE_OPTS $CALL_OPTS $JOBSTORE_NAME "${OUTSTORE_NAME}/call-${NAME}" ${INDEX_BASE}.xg $SAMPLE "s3://${OUTSTORE_NAME}/map-${NAME}/${SAMPLE}_${CHR_PREFIX}"
fi

if [ "${EVAL_OPTS}" != "SKIP" ]
then
	 ./eval-hgsvc.sh $HEAD_NODE_OPTS $EVAL_OPTS $JOBSTORE_NAME "$OUTSTORE_NAME/eval-${NAME}" $TRUE_VCF "s3://${OUTSTORE_NAME}/call-${NAME}/${SAMPLE}.vcf.gz" $REGIONS $SAMPLE
fi
